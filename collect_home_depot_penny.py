...
def classify(current, previous=None, disappeared=False):
    ...
    if previous:
        ...
        if price >= 0 and old_price > price:
            ...
            if drop >= 50:
                score += 10
                reasons.append(f"Price dropped {drop:.0f}% since previous observation")
    score = min(99, score)
...
def run(observations_path=OBSERVATIONS, history_path=HISTORY, output_path=OUTPUT):
    ...
    for item in observations:
        ...
        result = dict(item)
        result.update({"retailer": "Home Depot", "signalStatus": status, "signalScore": score,
                       "signalReasons": reasons, "requiresPhysicalConfirmation": status != "CONFIRMED",
                       "observedAt": item.get("observedAt") or now})
    ...
    if complete:
        for key, entry in list(products.items()):
            ...
            item.update({"retailer": "Home Depot", "signalStatus": status, "signalScore": score,
                         "signalReasons": reasons, "requiresPhysicalConfirmation": True, "observedAt": now})
            results.append(item)
    ...
    output = {"status": "ok", "experimental": True, "retailer": "Home Depot",
              "engineVersion": ENGINE_VERSION, "updatedAt": now, "captureComplete": complete,
